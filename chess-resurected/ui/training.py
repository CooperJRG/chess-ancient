...
PRESETS = {
    ...
    "serious": {"games": 10000, "cycle_games": 500, "max_cycles": 0, "max_ply": 180, "mcts_simulations": 200, "train_epochs": 2, "batch_size": 512, "num_blocks": 8, "channels": 192, "ladder_games": 100},
}
...
def reset_state() -> None:
    global state
    with _lock:
        state = PipelineState(device_name=str(_settings.get("device", DEFAULT_SETTINGS["device"])))
...
def _run_mcts_pipeline(settings: dict, paths: dict[str, Path]) -> None:
    ...
    cycle = 1
    engine = UciEngine(str(settings["engine_path"]))
    try:
        existing_samples, existing_meta = load_replay(paths["latest_replay"])
        _push({"replay_samples": len(existing_samples)})
        _log(
            f"Started MCTS {'infinite' if infinite else 'finite'} run '{settings['run_name']}' "
            f"on {settings['device']} with {settings['cycle_games']} games/cycle"
        )
        ...
        while not _stop_requested() and (infinite or cycle <= max_cycles):
            ...
            _push({"replay_samples": len(existing_samples), "records_filtered": len(existing_samples)})
            if _stop_requested():
            ...
            def _train_progress(metrics: dict) -> None:
                _stage_progress(
                    ...
                    span=0.34,
                    started=train_start,
                    extra={"train_loss": float(metrics.get("total_loss", 0.0))},
                )
                _log(
            ...
            _push({"train_loss": float(last.get("total_loss", 0.0)), "last_checkpoint_at": time.time()})
...
def _run_pipeline() -> None:
    ...
    try:
        if str(settings.get("selfplay_mode", "mcts")) == "mcts":
