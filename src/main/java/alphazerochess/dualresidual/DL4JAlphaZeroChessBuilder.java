...
class DL4JAlphaZeroChessBuilder {
    ...
    public DL4JAlphaZeroChessBuilder(int[] kernel, int[] strides, ConvolutionMode mode) {
    ...
    public DL4JAlphaZeroChessBuilder() {
        this(new int[] {3, 3},  new int[] {1, 1}, ConvolutionMode.Same);

        this.conf =  new NeuralNetConfiguration.Builder()
                .updater(new Sgd())
                .weightInit(WeightInit.LECUN_NORMAL)
                .graphBuilder().setInputTypes(InputType.convolutional(8, 8, 14));
    }
    ...
    /**
     * Value head for chess, estimates how valuable the current
     * board position is based on the label.
     */
    public String addChessValueHead(String inName, boolean useActivation) {
        String convName = "chess_value_head_conv_";
        String bnName = "chess_value_head_batch_norm_";
        String actName = "chess_value_head_relu_";
        String denseName = "chess_value_head_dense_";
        String outputName = "chess_value_head_output_";
        ...
        conf.addLayer(outputName, new OutputLayer.Builder(LossFunctions.LossFunction.MSE).activation(Activation.IDENTITY).nIn(256).nOut(1).build(), denseName);
        return outputName;
    }
